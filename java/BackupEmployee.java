...
public class BackupEmployee {
	...
	public static void main(String[] args) {
		...
		sql1 = "select * from testemployee order by id";// SQL语句

		sql2 = "select * from testemployee1 order by id";//

		// sql = "select * from view_emp where iccardid = '0313240578'";
		// sql2 = "INSERT INTO `testemployee`(`ID`, `Name`, `depid`,
		// `depname`,`cardid`,`direct`,`costID` ) VALUES (?,?,?,?,?,?,?)";
		// sql2 = "INSERT INTO `testemployee`(`ID`, `Name`, `depid`, `depname`,
		// `cardid`) VALUES
		// ('133028','徐文勇','7933','策略-資源管理-數據-系統整合課','0313240578')";
		// sql3 = "DELETE FROM `testemployee`";
		// sql4 = "UPDATE `testemployee` SET `Permission` = '1' WHERE ID IN
		// ('01394','208648','40908','68951','CY439','KQ933','KV447','N6193','R0405','R5632','G0956','403935','U2782','WL930','R1004','286943','T9220','D3702','K7804','N5400','58788','FE440','G8611','N6398','N6315','N5289','N3447','A5798','G6659','156339','03236','AW050','04664','F0025','K4230','F0660','KQ728','256176','DC705','U1939','K7380','194236','175418','565535','368364','N5397','426017')";
		...
		sql_countNew = "select count(*) from testemployee order by id";
		sql_countOld = "select count(*) from testemployee1 order by id";
		db1 = new DBHelper(sql1);// 创建DBHelper对象 |SelectCRUD Create result
									// Delete
		...
		try {
			...
			JSONObject unames = new JSONObject();
			JSONObject udepnames = new JSONObject();
			JSONObject udepids = new JSONObject();
			JSONObject ucardids = new JSONObject();
			JSONObject udirects = new JSONObject();
			JSONObject ucostids = new JSONObject();
			...
			JSONObject unames2 = new JSONObject();
			JSONObject udepnames2 = new JSONObject();
			JSONObject udepids2 = new JSONObject();
			JSONObject ucardids2 = new JSONObject();
			JSONObject udirects2 = new JSONObject();
			JSONObject ucostids2 = new JSONObject();

			// String userID = "123";
			// String employeeName = "test";
			// JSONObject createrMember = new JSONObject();
			// createrMember.put("userid", userID);
			// createrMember.put("employeeName", employeeName);
			while (rsNew.next()) {// 判断是否还有下一个数据
				...
				uids[i] = uid;
				unames.put(uid, uname);
				udepnames.put(uid, udepname);
				udepids.put(uid, udepid);
				ucardids.put(uid, ucardid);
				udirects.put(uid, udirect);
				ucostids.put(uid, ucostid);
				// unames[i] = uname;
...
//				System.out.println(uid + "\t" + uname + "\t" + udepname + "\t" + udepid + "\t" + ucardid);
			}
			...
			while (rsOld.next()) {
				...
				uids2[j] = uid;
				unames2.put(uid, uname);
				udepnames2.put(uid, udepname);
				udepids2.put(uid, udepid);
				ucardids2.put(uid, ucardid);
				udirects2.put(uid, udirect);
				ucostids2.put(uid, ucostid);
			...
			for (int k = 0; k < i; k++) {
				...
				if (contains(uids2, uids[k]) == true) {
					if (!unames2.get(uids[k]).equals(unames.get(uids[k]))
							|| !udepnames2.get(uids[k]).equals(udepnames.get(uids[k]))
							|| !udepids2.get(uids[k]).equals(udepids.get(uids[k]))
							|| !ucardids2.get(uids[k]).equals(ucardids.get(uids[k]))
							|| !udirects2.get(uids[k]).equals(udirects.get(uids[k]))
							|| !ucostids2.get(uids[k]).equals(ucostids.get(uids[k]))) {
						System.out.println(unames2.get(uids[k])+""+unames.get(uids[k]));
						x++;
